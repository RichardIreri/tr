...
class APITestCase(unittest.TestCase):
    ...
    def test_bad_auth(self):
        ...
        u = User(email='radioampex@gmail.com', password='cat', confirmed=True,
                 role=r)
        db.session.add(u)
        db.session.commit()
        ...
        response = self.client.get(
            '/api/v1/posts/',
            headers=self.get_api_headers('radioampex@gmail.com', 'dog'))
        self.assertEqual(response.status_code, 401)
    ...
    def test_unconfirmed_account(self):
        ...
        r = Role.query.filter_by(name='User').first()
        self.assertIsNotNone(r)
        u = User(email='radioampex@gmail.com', password='cat', confirmed=False,
                 role=r)
        db.session.add(u)
        db.session.commit()
    ...
    def test_users(self):
        ...
        r = Role.query.filter_by(name='User').first()
        self.assertIsNotNone(r)
        u1 = User(email='radioampex@gmail.com', username='radio',
                  password='cat', confirmed=True, role=r)
        u2 = User(email='richardireri19@gmail.com', username='richard',
        ...
        response = self.client.get(
            ...
            headers=self.get_api_headers('richardireri19@gmail.com', 'dog'))
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual(json_response['username'], 'radio')
        response = self.client.get(
            '/api/v1/users/{}'.format(u2.id),
            headers=self.get_api_headers('richardireri19@gmail.com', 'dog'))
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual(json_response['username'], 'richard')
    ...
    def test_comments(self):
        ...
        response = self.client.post(
            ...
            headers=self.get_api_headers('richardireri19@gmail.com', 'dog'),
            data=json.dumps({'body': 'Good [post](http://gmail.com)!'}))
        self.assertEqual(response.status_code, 201)
        json_response = json.loads(response.get_data(as_text=True))
        url = response.headers.get('Location')
        self.assertIsNotNone(url)
        self.assertEqual(json_response['body'],
                        'Good [post](http://gmail.com)!')
        self.assertEqual(
